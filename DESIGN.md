...
1. 实现顺序固定为 MkDocs 默认能力、Material 官方能力、最小内容级扩展。
2. 正文优先，主文字与背景对比不低于 4.5:1。
3. 语义差异同时使用颜色和线型或形状表达。
4. 站点不维护自定义动效，交互反馈使用 Material 原生行为。
5. 键盘焦点、主题切换和响应式导航由 Material 管理。
6. 自定义样式只解决正文内容无法由主题表达的问题，不复制官方组件。
...
- 导航、页内目录、搜索、Cards、表格和代码块使用 Material 官方响应式行为。
- 自定义教学图必须随正文宽度缩放，不制造页面级横向滚动。
- 移动端不内嵌浏览器 PDF 阅读器；保留直接打开和下载入口。
- 每次修改内容级样式后，至少实测 375px 手机、768px 平板和桌面宽度。
